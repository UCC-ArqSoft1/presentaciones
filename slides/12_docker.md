...
CMD ["./server"]
```
3. Modifica el código de golang para que en lugar de decir `router.Run("localhost:8080")` diga `router.Run(":8080")`
...
1. Crear un archivo **Dockerfile** en la carpeta raiz del ejercicio 7 de vinilos (version CON BD)
2. En el archivo escribe:
```bash
FROM golang:1.24
...
4. Vamos a la carpeta **ej-react/frontend**
5. Creamos un archivo **Dockerfile** y escribimos:
```bash
FROM node:22
...
7. Crear un archivo **docker-compose.yml** en la carpeta raíz del proyecto.
...
9. Si intentamos ejecutar, fallará, porque no se le brinda suficiente tiempo a la BD para que se instale. Por eso agregaremos una función en go:
```go
...
10. Ejecutar en la terminal: 
```bash
docker-compose up --build
```
11. Ingresa a `http://localhost:5173/` para verificar que Vite responde correctamente.`
12. Con postman asegurarse de que los servicios de GO responden
